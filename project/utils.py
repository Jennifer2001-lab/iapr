...
def save(index, mask, pieces, feature_map, labels):
    cluster = []

    for label in np.unique(labels):
        if label == -1:
            continue
        cluster.append([pieces[idx] for idx in np.where(labels == label)[0]])

    cluster.append([pieces[idx] for idx in np.where(labels == -1)[0]])

    solved_puzzle = [
        np.zeros((128 * 3, 128 * 4, 3)) + np.random.randint(0, 254, size=3)
        for _ in range(3)
    ]

    solution = [mask, feature_map, cluster, solved_puzzle]
    export_solutions(index, solution, path="data_project", group_id="13")
